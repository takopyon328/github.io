...
### (2.5) 文単位分割(`--split-sentences`、推奨)

ファイル全体を 1 発話として MFA に渡すと、テキストと音声の不一致 1 箇所で
ファイル全体のアラインメントが失敗する。`--split-sentences` を指定すると:

1. テキストを文末記号(。!?)・改行で文に分割(各 AP に sentence_index を付与)
2. 音声の無音区間(≥0.3 秒)を RMS エネルギーで検出
3. 累積モーラ数に比例した文境界の予測時刻と無音候補を、順序を保った動的計画法で
   対応付け(±3 秒以内)。対応する無音がない文境界は分割せず同一発話にまとめる
4. 発話ごとに音声を切り出して MFA に渡し、結果の時刻をオフセットして戻す
5. 失敗した発話はスキップし、該当句は時刻なし・low_confidence で出力(部分的成功)
