...
| `--split-sentences` | off | 文末記号(。!?・改行)と無音検出でファイルを文単位に分割してアラインメントする。**通常のアラインメントが失敗するファイルへのフォールバック用**。文境界と無音の対応付けに失敗すると境界がずれるため、使用時は結果を Praat で照合すること(不自然な発話は low_confidence+警告で通知され、対応付けが疑わしい場合はファイル全体方式へ自動フォールバックする) |
